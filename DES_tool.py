...
def encryptor(plain, key):
    ...
    return {"cipher": _2hex(cipher), "addition": str(encrypt_package["addition_count"])}

def decryptor(cipher, addition, key):
    # 编码：
    k = _2bin(key)
    # 裁剪：
    decrypt_package = cut(_2bin(cipher), 64)
    plain = ""
    if (decrypt_package['addition_count']!=0):
        return {"err": "Error: Bits check failed, maybe the cipher you've input was wrong?"}
    else:
        for d in decrypt_package["data"]:
            node = DES.decrypt(d, k, silence=True)
            plain += node
